...
describe('release contract', () => {
  it('targets the Ashes of Pantheon GitHub release feed', async () => {
    ...
    expect(packageJson.build.artifactName).toBe('Ashes-of-Pantheon-QA-Tool-${version}-win-x64.${ext}')
    expect(packageJson.build.win?.target).toBe('zip')
    expect(packageJson.build.nsis).toBeUndefined()
    expect(packageJson.dependencies['electron-updater']).toBeUndefined()
    expect(packageJson.scripts['build:win']).toContain('--win --dir --x64')
    expect(packageJson.scripts['build:win']).toContain('node scripts/package-release.mjs')
  ...
  it('publishes Windows update artifacts for v tags', async () => {
    ...
    expect(workflow).toContain('release/artifacts/update.json')
    expect(workflow).toContain('release-previous')
    expect(workflow).toContain('$index.deltas')
